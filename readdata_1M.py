...
MAX_SAMPLES = 1000000      # Maximum samples to collect before stopping
BYTES_PER_SAMPLE = 7
...
class SerialPlotter:
    def __init__(self, port, baudrate, max_samples):
        self.port = port
        ...
        self.ax.set_title(f'Live IIS3DWB Acceleration Data (Max {max_samples:,} samples)')
    ...
    def parse_new_samples(self):
        ...
        new_samples = 0
        while len(self.buffer) >= BYTES_PER_SAMPLE and self.sample_count < self.max_samples:
            ...
            if self.sample_count >= self.max_samples:
                self.running = False
                print(f"Reached maximum samples ({self.max_samples:,}). Stopping data collection.")
                break
    ...
    def animate(self, frame):
        ...
        if time.time() - self.last_data_time > 10:  # 10 second timeout
        ...
        self.sample_text.set_text(f'Samples: {self.sample_count:,}/{self.max_samples:,}')
    ...
    def start_plotting(self):
        ...
        print("Starting live plot... Will stop after collecting 10,000 samples")
        ...
        try:
            ...
            print(f"Final count: {self.sample_count:,} samples collected")

def parse_samples(raw_bytes):
    """Legacy function for parsing a buffer of samples (kept for compatibility)"""
    xs, ys, zs = [], [], []
...
def main():
    ...
    print(f"Starting live serial plotter (10K sample limit)...")
    print(f"Port: {SERIAL_PORT}")
    print(f"Baudrate: {BAUDRATE}")
    print(f"Max samples: {MAX_SAMPLES:,}")
    
    plotter = SerialPlotter(SERIAL_PORT, BAUDRATE, MAX_SAMPLES)
